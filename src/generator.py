...
class VerilogGenerator:
    ...
    @staticmethod
    def _parse_multimodule(raw: str, top_name: str) -> dict[str, str]:
        """
        Split LLM output into individual modules.

        Recognizes two formats:
        1. Explicit markers: "//--- MODULE: <name> ---"
        2. Fallback: splits on "module <name>" declarations
        """
        ...
        # Try explicit marker format first
        _MARKER_RE = re.compile(r"^//---\s*MODULE:\s*(\w+)\s*---", re.MULTILINE)
        ...
        if markers:
            ...
            for i, m in enumerate(markers):
                ...
                if code:
                    modules[name] = code
            return modules if modules else {top_name: raw.strip()}

        # Fallback: split on "module <name>" declarations
        _MODULE_RE = re.compile(
            r"((?://[^\n]*\n)*\s*module\s+(\w+)\s*[\(#].*?endmodule)",
            re.DOTALL,
        )
        found = _MODULE_RE.findall(raw)
        if len(found) > 1:
            return {name: code.strip() for code, name in found}

        # Single module — return as-is
        return {top_name: raw.strip()}
    ...
    def generate_multimodule(
        ...
        """
        Generate a hierarchical Verilog design (top + submodules).

        Returns:
            dict mapping module name → Verilog source code.
            The top-level module's key matches `module_name`.
        """
        prompt_cfg = self.config["multimodule_generation"]
        user_prompt = prompt_cfg["user_template"].format(
            description=description,
            module_name=module_name,
            inputs=", ".join(inputs),
            outputs=", ".join(outputs),
            submodules=submodules or "Auto-decide submodule decomposition",
            requirements=requirements or "None specified",
        )
        raw = self._call_llm(prompt_cfg["system"], user_prompt)
        return self._parse_multimodule(raw, module_name)
    ...
    def _mock_response(self, prompt: str) -> str:
        ...
        if re.search(r"Submodule guidance:", prompt, re.I):
            return self._mock_multimodule(prompt)
    ...
    def _mock_multimodule(self, prompt: str) -> str:
        """Mock: generate a hierarchical full_adder design (half_adder + full_adder)."""
        m = re.search(r"Top module name:\s*(\w+)", prompt)
        top_name = m.group(1) if m else "full_adder"

        inp_m = re.search(r"Inputs:\s*(.+)", prompt)
        out_m = re.search(r"Outputs:\s*(.+)", prompt)
        raw_inputs = [p.strip() for p in inp_m.group(1).split(",")] if inp_m else ["a", "b", "cin"]
        raw_outputs = [p.strip() for p in out_m.group(1).split(",")] if out_m else ["sum", "cout"]

        # Generate a full-adder from two half-adders as the canonical multi-module mock
        half_adder = (
            "//--- MODULE: half_adder ---\n"
            "// Auto-generated by AI-EDA Playground (mock mode)\n\n"
            "module half_adder (\n"
        ...
        # Build full_adder ports from user-specified I/O
        in_names = [self._parse_port(p)[1] for p in raw_inputs]
        out_names = [self._parse_port(p)[1] for p in raw_outputs]
        a_sig = in_names[0] if len(in_names) > 0 else "a"
        b_sig = in_names[1] if len(in_names) > 1 else "b"
        cin_sig = in_names[2] if len(in_names) > 2 else "cin"
        sum_sig = out_names[0] if len(out_names) > 0 else "sum"
        cout_sig = out_names[1] if len(out_names) > 1 else "cout"

        full_adder = (
            f"//--- MODULE: {top_name} ---\n"
            f"// Auto-generated by AI-EDA Playground (mock mode)\n\n"
            f"module {top_name} (\n"
            f"  input  wire {a_sig},\n"
            f"  input  wire {b_sig},\n"
            f"  input  wire {cin_sig},\n"
            f"  output wire {sum_sig},\n"
            f"  output wire {cout_sig}\n"
            f");\n"
            f"  wire s1, c1, c2;\n\n"
            f"  half_adder ha1 (.a({a_sig}), .b({b_sig}), .sum(s1), .carry(c1));\n"
            f"  half_adder ha2 (.a(s1), .b({cin_sig}), .sum({sum_sig}), .carry(c2));\n\n"
            f"  assign {cout_sig} = c1 | c2;\n"
            f"endmodule\n"
        )
